...
matrix_t *matrix_sum_openmp(matrix_t *A, matrix_t *B, int n){
    //Não podemos somar
    if( (A->rows != B->rows) && (A->cols != B->cols) ){
        printf("Impossível somar\n");
        return NULL;
    ...
    #pragma omp parallel for num_threads(n) shared(A, B) private(j)
    for(i = 0; i < A->rows; i++){
        for(j = 0; j < A->cols; j++){
            A->data[i][j] += B->data[i][j];
        }
...
matrix_t *matrix_multiply_openmp(matrix_t *A, matrix_t *B, matrix_t *C, int n){
    //Não podemos multiplicar
    if(A->cols != B->rows){
        printf("Impossível multiplicar\n");
        printf("m1: %d | m2: %d\n", A->cols, B->rows);
    ...
    int i,j,k;
    #pragma omp parallel for num_threads(n) shared(A, B, C) private(j,k)
    for(i = 0; i < A->rows; i++){
        for(j = 0; j < B->cols; j++){
            for(k = 0; k < A->cols; k++){
                C->data[i][j] += A->data[i][k] * B->data[k][j];
...
matrix_t* matrix_sort_openmp(matrix_t* m, int nthreads){
    int size = m->rows * m->cols;

    iterative_merge_sort_openmp(m->data[0], 0, size-1, nthreads);
    return m;
}
...
void iterative_merge_sort_openmp(double* vet, int begin, int end, int n){
    int current; 
    int left, mid, right;
    ...
    for (current = 1; current < size; current = 2 * current){
        #pragma omp parallel for shared(vet) num_threads(n) private(left, mid, right)
        ...
        for (left = begin; left < end; left += 2 * current) {
            ...
            merge_openmp(vet, left, mid, right);
        } 
    }
}

void merge_openmp(double *vet, int l, int m, int r){   
    long int i, j, k;
    int n1 = m - l + 1;
    int n2 =  r - m;
